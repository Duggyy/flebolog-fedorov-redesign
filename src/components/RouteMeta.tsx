...
/**
 * `<h1>`, который мы закоммитили в прошлый раз.
 *
 * Нужен, чтобы отличить «новая страница ещё не отрисовалась» от «отрисовалась»:
 * при переходе Suspense держит на экране ПРЕДЫДУЩУЮ страницу, поэтому первые
 * тики чтения `<h1>` возвращают старый заголовок. Раньше он успевал «устояться»
 * за 3 тика (180 мс) и коммитился — а когда чанк наконец загружался, эффект уже
 * не перезапускался, и заголовок оставался от прошлой страницы до следующего
 * перехода.
 *
 * Проверено: переход «/» → `/conference-photos/riga-2018` на холодном кэше
 * оставлял во вкладке «ФЁДОРОВДмитрий Анатольевич — флеболог Д.А. Фёдоров»
 * навсегда, хотя `<h1>` на странице уже был «Рига - 2018 г.». Побочно от этого
 * зависел и `noindex`: переход с 404 на медленно грузящуюся страницу оставлял
 * `noindex` на нормальной странице.
 */
let committedH1 = "";
...
const RouteMeta = () => {
  ...
  useEffect(() => {
    ...
    if (pathname === "/") {
      ...
      setNoIndex(false);
      // Index подключён обычным импортом, поэтому его <h1> уже на экране.
      // Запоминаем его: без этого первый же переход с главной примет заголовок
      // главной за «устоявшийся» и оставит его вместо нового.
      const homeH1 = readH1();
      if (homeH1) committedH1 = homeH1;
      return;
    }
    ...
    const commit = (h1: string) => {
      // Запоминаем: именно этот <h1> теперь считается «уже показанным».
      committedH1 = h1;
    ...
    const tick = () => {
      ...
      const h1 = readH1();
      if (h1) {
        // Пока на экране предыдущая страница, её <h1> не должен считаться
        // «устоявшимся» — иначе закоммитим заголовок прошлого маршрута.
        const isPreviousPage = committedH1 !== "" && h1 === committedH1;
        if (isPreviousPage) {
          last = h1;
          stable = 0;
        } else if (h1 === last) {
      ...
      } else if (last && !(committedH1 !== "" && last === committedH1)) {
        // Чанк так и не загрузился. Фиксируем то, что успели увидеть, но не
        // возвращаем заголовок прошлой страницы: он уже не актуален.
        commit(last);
      }
    };
